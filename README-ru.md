# GitLab MCP Server

GitLab MCP — сервер Model Context Protocol, который предоставляет инструменты для работы с проектами GitLab, merge-request'ами и тегами непосредственно из Claude Code, Code CLI и других MCP-клиентов.

## Содержание

- [Возможности](#возможности)
- [Требования](#требования)
- [Установка](#установка)
  - [Используя npx (Рекомендуется)](#используя-npx-рекомендуется)
  - [Используя Claude MCP CLI](#используя-claude-mcp-cli)
  - [Ручная установка (Для разработки)](#ручная-установка-для-разработки)
- [Запуск сервера (stdio)](#запуск-сервера-stdio)
- [Конфигурация для Code (Рекомендуется)](#конфигурация-для-code-рекомендуется)
- [Конфигурация для Claude Code CLI](#конфигурация-для-claude-code-cli)
- [Конфигурация для VS Code Cline](#конфигурация-для-vs-code-cline)
- [MCP-инструменты](#mcp-инструменты)
  - [Режим только для чтения vs Режим записи](#режим-только-для-чтения-vs-режим-записи)
  - [Руководство по использованию инструментов](#руководство-по-использованию-инструментов)
- [Формат ответа](#формат-ответа)

## Возможности

- Получение списка доступных проектов GitLab и подробной информации по каждому проекту.
- Просмотр тегов репозитория с расчётом следующего релизного тега по SemVer.
- Списки merge-request'ов с фильтрами по состоянию и дате обновления.
- Детальная информация о merge-request'ах с флагом «свежести» (смерджено менее 24 часов назад).
- Просмотр измененных файлов и полных diff'ов для merge-request'ов с пагинацией и фильтрацией.
- Поиск по merge-request'ам (название и описание).
- Список пользователей GitLab с отслеживанием активности и batch-операциями.
- Управление участниками проектов и групп с информацией об уровнях доступа.
- Мониторинг CI/CD пайплайнов с фильтрами по статусу и диапазонам дат.
- Просмотр задач пайплайнов с деталями выполнения, артефактами и информацией о раннерах.
- Отслеживание истории сборок и анализ ошибок в задачах по проектам.

## Требования

- Node.js ≥ 20
- Переменные окружения:
  - `GITLAB_URL` — базовый URL инстанса GitLab
  - `GITLAB_TOKEN` — персональный токен доступа
    - **Режим только для чтения (по умолчанию)**: права `read_api` и `read_repository`
    - **Режим записи** (для создания тегов): право `api`
  - `GITLAB_READ_ONLY` — опциональная, по умолчанию `true` для безопасности
    - Установите в `false` для включения операций записи (создание тегов)
  - `GITLAB_TIMEZONE` — опциональная, часовой пояс для форматирования дат и времени (по умолчанию `Europe/Moscow`)
    - Примеры: `UTC`, `America/New_York`, `Asia/Tokyo`, `Europe/London`
    - Все даты в ответах API будут отформатированы в указанной временной зоне

## Установка

### Используя npx (Рекомендуется)

Вы можете запустить сервер напрямую через npx без установки:

```bash
GITLAB_URL="https://gitlab.example.com" \
GITLAB_TOKEN="glpat-your-token-here" \
npx -y @vitalyostanin/gitlab-mcp@latest
```

### Используя Claude MCP CLI

Установка через Claude MCP CLI:

```bash
claude mcp add --scope user \
--env GITLAB_URL='https://gitlab.example.com' \
--env GITLAB_TOKEN='glpat-your-token-here' \
gitlab-mcp -- npx -y @vitalyostanin/gitlab-mcp@latest
```

**Опции области видимости:**
- `--scope user`: Установка для текущего пользователя (для всех проектов)
- `--scope project`: Установка только для текущего проекта

**Удаление:**

```bash
claude mcp remove gitlab-mcp --scope user
```

### Ручная установка (Для разработки)

```bash
npm install
npm run build
```

## Запуск сервера (stdio)

```bash
GITLAB_URL="https://gitlab.example.com" \
GITLAB_TOKEN="glpat-example-token" \
node dist/index.js
```

## Конфигурация для Code (Рекомендуется)
Добавьте в `~/.code/config.toml`:
```toml
[mcp_servers.gitlab-mcp]
command = "npx"
args = ["-y", "@vitalyostanin/gitlab-mcp@latest"]

[mcp_servers.gitlab-mcp.env]
GITLAB_URL = "https://gitlab.example.com"
GITLAB_TOKEN = "glpat-your-token-here"
GITLAB_TIMEZONE = "Europe/Moscow"
```

## Конфигурация для Claude Code CLI

Чтобы использовать этот MCP сервер с [Claude Code CLI](https://github.com/anthropics/claude-code), вы можете:

1. **Использовать Claude MCP CLI** - смотрите секцию [Установка](#установка) выше
2. **Ручная конфигурация** - добавьте в файл `~/.claude.json`:

```json
{
  "mcpServers": {
    "gitlab-mcp": {
      "type": "stdio",
      "command": "npx",
      "args": ["-y", "@vitalyostanin/gitlab-mcp@latest"],
      "env": {
        "GITLAB_URL": "https://gitlab.example.com",
        "GITLAB_TOKEN": "glpat-your-token-here",
        "GITLAB_TIMEZONE": "Europe/Moscow",
        "GITLAB_USE_STRUCTURED_CONTENT": "false"
      }
    }
  }
}
```

**Примечание:** Эта конфигурация использует npx для запуска опубликованного пакета. Для локальной разработки используйте `"command": "node"` с `"args": ["/абсолютный/путь/к/gitlab-mcp/dist/index.js"]`.

## Конфигурация для VS Code Cline

Чтобы использовать этот MCP сервер с расширением [Cline](https://github.com/cline/cline) в VS Code:

1. Откройте VS Code с установленным расширением Cline
2. Нажмите на иконку MCP Servers в верхней навигации Cline
3. Выберите вкладку "Configure" и нажмите "Configure MCP Servers"
4. Добавьте следующую конфигурацию в `cline_mcp_settings.json`:

```json
{
  "mcpServers": {
    "gitlab-mcp": {
      "command": "npx",
      "args": ["-y", "@vitalyostanin/gitlab-mcp@latest"],
      "env": {
        "GITLAB_URL": "https://gitlab.example.com",
        "GITLAB_TOKEN": "glpat-your-token-here",
        "GITLAB_TIMEZONE": "Europe/Moscow",
        "GITLAB_USE_STRUCTURED_CONTENT": "false"
      }
    }
  }
}
```

**Примечание:** Эта конфигурация использует npx для запуска опубликованного пакета. Для локальной разработки используйте `"command": "node"` с `"args": ["/абсолютный/путь/к/gitlab-mcp/dist/index.js"]`. Переменные окружения `GITLAB_TIMEZONE` и `GITLAB_READ_ONLY` являются опциональными.

## MCP-инструменты

| Инструмент | Описание |
| --- | --- |
| `service_info` | Статус подключения к GitLab и активные фильтры. |
| `gitlab_projects` | Список доступных проектов с пагинацией, поиском и фильтрацией по архиву. |
| `gitlab_project_details` | Подробности проекта по ID или namespace. |
| `gitlab_project_tags` | Теги проекта с расчётом следующего релизного тега по SemVer и готовой ссылкой для создания. |
| `gitlab_project_tag_create` | Создание нового тега в репозитории проекта (требуется доступ на запись). |
| `gitlab_projects_search` | Поиск проектов по имени, пути и описанию. |
| `gitlab_merge_requests` | Merge-request'ы проекта с фильтрами и пагинацией. |
| `gitlab_merge_request_details` | Детали MR, включая ссылки и флаг свежести. |
| `gitlab_merge_request_changes` | Список измененных файлов с фильтрацией и пагинацией. |
| `gitlab_merge_request_diff` | Полный diff-контент (может возвращать большие данные). |
| `gitlab_merge_requests_search` | Поиск merge-request'ов по названию и описанию. |
| `gitlab_users` | Список пользователей с пагинацией и отслеживанием активности. |
| `gitlab_user_details` | Подробная информация о пользователе по ID или username. |
| `gitlab_users_batch` | Эффективное получение информации о нескольких пользователях (макс. 50). |
| `gitlab_current_user` | Информация о текущем пользователе (владелец токена). |
| `gitlab_project_members` | Список участников проекта с уровнями доступа. |
| `gitlab_group_members` | Список участников группы с уровнями доступа. |
| `gitlab_pipelines` | Список пайплайнов проекта с фильтрами и пагинацией (ref, status, scope, интервалы created/updated, sha). |
| `gitlab_pipeline_details` | Подробная информация о пайплайне по проекту и ID пайплайна. |
| `gitlab_pipeline_jobs` | Список задач конкретного пайплайна с фильтрами. |
| `gitlab_pipeline_variables` | Список переменных пайплайна (в brief-режиме только ключи). |
| `gitlab_pipeline_test_report` | Краткий отчёт по тестам пайплайна (в brief — только суммарные метрики). |
| `gitlab_project_jobs` | Список всех задач проекта с фильтрами по статусу. |
| `gitlab_job_details` | Подробная информация о задаче по проекту и ID задачи. |
| `gitlab_job_trace` | Ссылка и краткая информация о логе задачи (build log). |
| `gitlab_job_trace_download` | Загрузка части лога задачи по HTTP Range с ограничением размера. |
| `gitlab_latest_pipeline` | Получение последнего пайплайна для ветки (по умолчанию: ветка по умолчанию). |

| `gitlab_commits` | Список коммитов проекта с фильтрами и пагинацией (по умолчанию brief). |
| `gitlab_commit_details` | Детальная информация о конкретном коммите по SHA. |
| `gitlab_commit_diff` | Файлы diff для коммита (по умолчанию brief; полный diff опционально). |
| `gitlab_commit_statuses` | Список CI-статусов коммита (по контекстам/пайплайнам). |

### Режим только для чтения vs Режим записи

По умолчанию сервер GitLab MCP работает в **режиме только для чтения** для безопасности. Это предотвращает случайные изменения в ваших репозиториях GitLab.

#### Режим только для чтения (по умолчанию)

- **Требования к токену**: права `read_api` и `read_repository`
- **Доступные операции**: Все операции чтения (список проектов, MR, тегов, пользователей и т.д.)
- **Конфигурация**: Специальная конфигурация не требуется (поведение по умолчанию)

#### Режим записи

Для включения операций записи (в настоящее время: создание тегов), необходимо:

1. **Сгенерировать новый токен GitLab** с правом `api` (вместо `read_api`)
   - Перейдите в GitLab → Настройки → Токены доступа
   - Создайте новый токен с правом `api`
   - Примечание: Право `api` включает разрешения на чтение и запись

2. **Установить переменную окружения** `GITLAB_READ_ONLY=false`

3. **Убедиться в наличии необходимых прав**: Вам нужна как минимум роль **Developer** в целевом проекте для создания тегов

**Пример конфигурации для режима записи:**

```toml
# ~/.code/config.toml
[mcp_servers.gitlab-mcp]
command = "npx"
args = ["-y", "@vitalyostanin/gitlab-mcp@latest"]
env = {
  "GITLAB_URL" = "https://gitlab.example.com",
  "GITLAB_TOKEN" = "glpat-your-api-scope-token",
  "GITLAB_READ_ONLY" = "false",
  "GITLAB_TIMEZONE" = "Europe/Moscow"
}
```

**Примечание по безопасности**: Режим записи позволяет серверу вносить изменения в ваши репозитории. Включайте его только при необходимости и убедитесь, что ваш токен надёжно защищён.

### Руководство по использованию инструментов

#### Инструменты для работы с проектами

- **`gitlab_projects`**: Получение списка доступных проектов с опциональными фильтрами (membership, статус архива, базовый поиск). Использует endpoint `/api/v4/projects`. **Применение**: Просмотр доступных проектов, выбор только активных проектов (`archived: false`) или анализ архивных репозиториев (`archived: true`). Каждый проект содержит флаг `archived` вместе с остальными полями.

- **`gitlab_projects_search`**: Поиск конкретных проектов по ключевым словам через GitLab Search API. Использует endpoint `/api/v4/search?scope=projects`. **Применение**: Поиск конкретных проектов по имени или пути (например, "telegram/service").

**Ключевое различие**: `gitlab_projects` предназначен для листинга/просмотра с фильтрами, в то время как `gitlab_projects_search` обеспечивает целевой поиск по ключевым словам через Search API.

#### Инструменты для работы с merge requests

- **`gitlab_merge_requests`**: Получение списка merge requests для конкретного проекта с фильтрами (состояние, диапазон дат). Требует указания идентификатора проекта. **Применение**: Получение всех MR для конкретного проекта, фильтрация по состоянию или дате обновления.

- **`gitlab_merge_requests_search`**: Поиск merge requests по тексту (заголовок, описание) через GitLab Search API. Может искать по всем проектам или в рамках конкретного проекта. **Применение**: Поиск MR по ключевым словам среди всех проектов.

**Ключевое различие**: `gitlab_merge_requests` выводит список MR для конкретного проекта, в то время как `gitlab_merge_requests_search` находит MR по текстовому поиску среди всех проектов.

##### Флаг свежести

**Флаг свежести** (freshness flag) — это булево поле, возвращаемое инструментом `gitlab_merge_request_details`, которое указывает, является ли merge request "свежим" или недавно смерженным:

- **Для несмерженных MR** (состояние `opened` или `closed`): `fresh = true` (всегда считаются свежими)
- **Для смерженных MR**:
  - `fresh = true` — смерджено в течение последних **24 часов**
  - `fresh = false` — смерджено более 24 часов назад

**Примеры использования:**
- Выявление недавно смерженных изменений, которые могут требовать внимания
- Отслеживание горячей/недавней активности merge requests в проекте
- Фильтрация старых смерженных MR при просмотре недавних изменений

**Пример ответа:**
```json
{
  "mergeRequest": {
    "id": 123,
    "iid": 45,
    "title": "Добавить новую функцию",
    "state": "merged",
    "mergedAt": "2025-10-13T10:30:00Z",
    "fresh": true
  }
}
```

#### Инструменты для работы с пользователями

- **`gitlab_users`**: Список всех пользователей GitLab с опциональными фильтрами (активные, заблокированные, поиск). Возвращает базовую информацию о пользователях, включая дату последней активности. **Применение**: Просмотр доступных пользователей, поиск пользователей по имени или username, мониторинг активности пользователей.

- **`gitlab_user_details`**: Получение подробной информации о конкретном пользователе по ID или username. Включает даты последней активности и входа. **Применение**: Получение деталей профиля пользователя, проверка статуса пользователя, просмотр истории активности.

- **`gitlab_users_batch`**: Получение подробной информации о нескольких пользователях одновременно (batch-режим, макс. 50 пользователей). Оптимизировано для массовых операций с параллельными API-запросами. Возвращает массив пользователей и массив notFound для отсутствующих пользователей. **Применение**: Эффективное получение информации о нескольких пользователях, анализ активности команды, массовое получение информации о пользователях.

- **`gitlab_current_user`**: Получение информации о текущем пользователе (владелец API токена). Включает флаги разрешений и статус 2FA. **Применение**: Проверка аутентификации, проверка разрешений текущего пользователя.

- **`gitlab_project_members`**: Список участников конкретного проекта с их уровнями доступа (Guest=10, Reporter=20, Developer=30, Maintainer=40, Owner=50). **Применение**: Проверка того, кто имеет доступ к проекту, просмотр уровней доступа, поиск мейнтейнеров проекта.

- **`gitlab_group_members`**: Список участников группы/namespace с их уровнями доступа (Guest=10, Reporter=20, Developer=30, Maintainer=40, Owner=50). **Применение**: Проверка членства в группе, просмотр уровней доступа группы, поиск администраторов группы.

**Ключевые возможности**:
- Все инструменты для работы с пользователями включают поля `lastActivityOn` и `lastSignInAt` для отслеживания активности
- Batch-режим поддерживает до 50 пользователей за один запрос для оптимальной производительности
- Корректно обрабатывает отсутствующих пользователей без сбоя всего batch-запроса
- Уровни доступа предоставляются как в числовом виде, так и в виде понятных описаний

#### Инструменты для работы с тегами

- **`gitlab_project_tags`**: Получение списка тегов репозитория с автоматической подсказкой следующей версии на основе SemVer. Возвращает все теги с информацией о коммитах, определяет последний тег, соответствующий SemVer, и предлагает следующую patch-версию. Также предоставляет готовую ссылку на GitLab UI для удобного создания тега вручную.

**Примеры использования:**
- Планирование следующей версии релиза
- Просмотр истории тегов с автоматической версионностью
- Получение быстрой ссылки для создания следующего релизного тега

**Как это работает:**
- Парсит все теги, соответствующие формату SemVer (например, `v1.2.3`, `1.0.0`)
- Определяет наивысшую версию как текущую
- Предлагает следующую patch-версию (например, `v1.2.3` → `v1.2.4`)
- Генерирует ссылку на GitLab UI: `https://gitlab.example.com/project/-/tags/new?tag_name=v1.2.4&ref=master`

**Пример ответа:**
```json
{
  "project": "namespace/my-project",
  "tags": [
    { "name": "v1.2.3", "commitId": "abc123...", "authoredAt": "2025-01-10T10:30:00Z" },
    { "name": "v1.2.2", "commitId": "def456...", "authoredAt": "2025-01-05T14:20:00Z" }
  ],
  "versionInfo": {
    "currentTag": "v1.2.3",
    "suggestedNextTag": "v1.2.4",
    "createTagUrl": "https://gitlab.example.com/namespace/my-project/-/tags/new?tag_name=v1.2.4&ref=master"
  },
  "pagination": {
    "page": 1,
    "perPage": 50,
    "total": 150,
    "hasMore": true
  }
}
```

- **`gitlab_project_tag_create`**: Создание нового тега в репозитории проекта GitLab программно. **Требуется режим записи** - см. раздел [Режим только для чтения vs Режим записи](#режим-только-для-чтения-vs-режим-записи) выше.

**Требования:**
- Включён режим записи (`GITLAB_READ_ONLY=false`)
- Токен GitLab с правом `api`
- Как минимум роль **Developer** в целевом проекте

**Параметры:**
- `project` (обязательный): ID проекта или путь (например, `"namespace/project"` или `123`)
- `tagName` (обязательный): Имя тега в формате SemVer (например, `"v1.2.4"` или `"1.2.4"`)
- `ref` (опциональный): Имя ветки или SHA коммита для тега (по умолчанию `"master"`)
- `message` (опциональный): Сообщение/аннотация тега
- `releaseDescription` (опциональный): Описание релиза (создаёт релиз в GitLab)

**Примеры использования:**
- Автоматизация создания тегов релизов в CI/CD пайплайнах
- Программное создание тегов после успешных сборок
- Массовое создание тегов в нескольких проектах

**Рекомендуемый рабочий процесс:**
1. Используйте `gitlab_project_tags` для проверки существующих тегов и получения предложенной следующей версии
2. Используйте `gitlab_project_tag_create` для создания нового тега
3. Обрабатывайте типичные ошибки (тег уже существует, недостаточно прав, ссылка не найдена)

**Пример использования:**
```json
{
  "project": "namespace/my-project",
  "tagName": "v1.2.4",
  "ref": "master",
  "message": "Релиз версии 1.2.4",
  "releaseDescription": "### Возможности\n- Добавлен новый API endpoint\n- Улучшена производительность"
}
```

**Пример ответа:**
```json
{
  "project": "namespace/my-project",
  "tag": {
    "name": "v1.2.4",
    "message": "Релиз версии 1.2.4",
    "target": "master",
    "commit": {
      "id": "abc123def456...",
      "message": "Merge branch 'feature/new-api'",
      "createdAt": "2025-10-14T10:30:00Z"
    },
    "release": {
      "tagName": "v1.2.4",
      "description": "### Возможности\n- Добавлен новый API endpoint\n- Улучшена производительность"
    },
    "url": "https://gitlab.example.com/namespace/my-project/-/tags/v1.2.4"
  }
}
```

**Типичные ошибки:**
- **Режим только для чтения**: Создание тегов отключено по умолчанию. Включите режим записи, как описано выше.
- **409 Conflict**: Тег уже существует. Проверьте существующие теги с помощью `gitlab_project_tags`.
- **403 Forbidden**: Недостаточно прав. Убедитесь, что у вас есть роль Developer+ и токен с правом `api`.
- **422 Unprocessable**: Ссылка (ветка/коммит) не найдена в репозитории.
- **400 Bad Request**: Неверный формат имени тега. Должен соответствовать SemVer (например, `v1.2.3`).

#### Инструменты для работы с пайплайнами и задачами

Инструменты для работы с пайплайнами и задачами предоставляют полный доступ к информации о CI/CD пайплайнах и задачах GitLab, позволяя отслеживать статус сборок, анализировать выполнение задач и просматривать историю пайплайнов.

- **`gitlab_pipelines`**: Список пайплайнов проекта с опциональными фильтрами (ref, статус, даты). Использует endpoint `/api/v4/projects/:id/pipelines`. **Применение**: Просмотр пайплайнов проекта, фильтрация по ветке или статусу (например, упавшие пайплайны на master), мониторинг активности CI/CD.

- **`gitlab_pipeline_details`**: Получение подробной информации о конкретном пайплайне. **Применение**: Просмотр полных деталей пайплайна включая длительность и покрытие, проверка временных меток выполнения, получение ссылки на пайплайн.

- **`gitlab_pipeline_jobs`**: Список всех задач в рамках конкретного пайплайна. **Применение**: Анализ этапов выполнения пайплайна, просмотр статусов и длительности задач, устранение неполадок в упавших шагах пайплайна.

- **`gitlab_project_jobs`**: Список всех задач по всем пайплайнам в проекте. **Применение**: Просмотр истории задач во всём проекте, фильтрация по статусу задачи (например, все упавшие задачи), анализ паттернов CI/CD.

- **`gitlab_job_details`**: Получение подробной информации о конкретной задаче. **Применение**: Просмотр деталей выполнения задачи, проверка артефактов и информации о раннере, анализ ошибок задачи.

- **`gitlab_latest_pipeline`**: Быстрый доступ к самому последнему пайплайну для ветки. **Применение**: Проверка текущего статуса CI/CD, получение результатов последней сборки, мониторинг здоровья ветки.

**Ключевые возможности**:
- Все инструменты для работы с пайплайнами и задачами поддерживают пагинацию (по умолчанию 50, макс. 100 на страницу)
- Фильтры по статусу для целевого поиска (например, 'failed', 'running', 'success')
- Полные метаданные включая длительность, временные метки и URL
- Интеграция с существующей системой идентификации проектов (по ID или пути)

**Статусы пайплайнов**:
- `created`, `waiting_for_resource`, `preparing`, `pending` - Пайплайн в очереди или запускается
- `running` - Пайплайн выполняется
- `success` - Все задачи завершены успешно
- `failed` - Одна или более задач упали
- `canceled` - Пайплайн был отменён вручную
- `skipped` - Пайплайн был пропущен (например, из-за правил ветки)
- `manual` - Пайплайн требует ручного запуска
- `scheduled` - Пайплайн запланирован к выполнению

**Статусы задач**:
- `created`, `pending` - Задача ожидает запуска
- `running` - Задача выполняется
- `success` - Задача завершена успешно
- `failed` - Задача упала
- `canceled` - Задача была отменена
- `skipped` - Задача была пропущена
- `manual` - Задача требует ручного запуска
- `scheduled` - Задача запланирована к выполнению

**Пример рабочего процесса**:
1. Список упавших пайплайнов: `gitlab_pipelines({ project: "my-project", status: "failed" })`
2. Получение деталей пайплайна: `gitlab_pipeline_details({ project: "my-project", pipelineId: 12345 })`
3. Проверка упавших задач: `gitlab_pipeline_jobs({ project: "my-project", pipelineId: 12345, scope: ["failed"] })`
4. Исследование конкретной задачи: `gitlab_job_details({ project: "my-project", jobId: 67890 })`

**Пример ответа (gitlab_pipelines)**:
```json
{
  "project": "namespace/my-project",
  "pipelines": [
    {
      "id": 12345,
      "iid": 678,
      "status": "failed",
      "ref": "master",
      "sha": "abc123de",
      "createdAt": "2025-10-14T10:00:00Z",
      "duration": 320,
      "webUrl": "https://gitlab.example.com/namespace/my-project/-/pipelines/12345"
    }
  ],
  "pagination": {
    "page": 1,
    "perPage": 50,
    "total": 150,
    "hasMore": true
  }
}
```

**Пример ответа (gitlab_job_details)**:
```json
{
  "project": "namespace/my-project",
  "job": {
    "id": 67890,
    "name": "test:unit",
    "stage": "test",
    "status": "failed",
    "duration": 45,
    "pipeline": {
      "id": 12345,
      "ref": "master",
      "status": "failed"
    },
    "artifacts": [
      {
        "fileType": "junit",
        "filename": "junit.xml",
        "size": 1024
      }
    ],
    "url": "https://gitlab.example.com/namespace/my-project/-/jobs/67890"
  }
}
```
## Формат ответа

Сервер по умолчанию возвращает ровно один узел данных, управляемый переменной `GITLAB_USE_STRUCTURED_CONTENT` (по умолчанию: `"true"`).

- При значении `true`: инструменты возвращают только узел MCP `structuredContent` с полными данными, а `content` — пустой массив `[]` (для соответствия типам MCP).
- При значении `false`: инструменты возвращают только узел MCP `content` — один текстовый элемент с JSON-строкой; узел `structuredContent` отсутствует.

Заметки для клиентов:
- Пользователям Claude Code можно установить `GITLAB_USE_STRUCTURED_CONTENT="false"`, если клиент ожидает данные в поле `content`.
- Пользователи Cline (VS Code) также могут установить флаг в `"false"` в конфигурации сервера при необходимости.
